...
### 5. CurriculumTermAgent (교과과정 용어 에이전트)

**역할**: 질문과 답변에서 사용되는 수학 용어의 적절성을 분석하고 교과과정에 맞게 검증

**주요 기능**:
- 질문 내 핵심 수학 개념 추출 및 용어 제안
- RAG(검색 증강 생성) 기반 교과과정 및 교과서 데이터베이스 검색
- 고등학교 1-3학년 수준에 맞는 용어 적절성 검증
- 답변 생성 시 부적절한 용어 식별 및 대안 제안

**통신 방식**:
- **입력**: Redis Streams (요청 수신)
- **출력**: Redis Streams (분석/검증 결과 반환)
